...
***********************************************************/
   public static ListNode reverseBuildNew(ListNode head)
   {
      ...
      for(ListNode i = head; i != null; i = i.getNext(   )) {
         reverse  = new ListNode(i.getValue(), reverse);
      }
      return reverse;
   }
...
prev and next.
   ********************************************************/
   public static ListNode iterateThreePointers(ListNode head) //go from 1->2->3->4 (head = 1) to 1<-2<-3<-4 (head = 4)
   {
      if(head == null){
         return null;
      }
...
********************************************************/
   ...
   {
      ListNode fin = pointerToLast(head);
      ListNode nextToLast = nextToLast(head);
      if(head.getNext() != null) {  
         nextToLast.setNext(null);
         return append(fin, recurPointersAppend(head));
      }
      return fin;
   }
